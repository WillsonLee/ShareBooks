    ...
    , m_currentDrawImageIndx(0)
{
    ...
    // 这里要设置的动画时间小于图片切换时间;
    m_opacityAnimation->setDuration(anim_time);
...
void CarouselImageWindow::initChangeImageButton()
{
    ...
    for (int i = 0; i < m_imageFileNameList.count(); i++)
    {
        ...
        pButton->setStyleSheet("QPushButton{border-image:url(:/icons/icons/not_selected.png);}\
                                QPushButton:checked{border-image:url(:/icons/icons/selected.png);}");
...
void CarouselImageWindow::startPlay()
{
    ...
    else if (m_imageFileNameList.count() > 1)
    {
        ...
        m_imageChangeTimer.start(time_out);
        update();
    }   
}
